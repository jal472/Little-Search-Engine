...
public class LittleSearchEngine {
		...
		while (scan.hasNext()){
			...
			if (key != null){
				if (map.containsKey(key)){
					Occurrence occurr = map.get(key);
					occurr.frequency++;
				}
				else{
					Occurrence ocurr = new Occurrence(docFile, 1);
					map.put(key, ocurr);
				}
			}
		}
	...
	public void mergeKeyWords(HashMap<String,Occurrence> kws) {
		for (String key : kws.keySet()){
			ArrayList<Occurrence> ocurr = new ArrayList<Occurrence>();

			if (keywordsIndex.containsKey(key))
				ocurr = keywordsIndex.get(key);
				
			ocurr.add(kws.get(key));
			insertLastOccurrence(ocurr);
			keywordsIndex.put(key, ocurr);
		}
	}
	
	private boolean containsACharacter(String word) //helper:returns boolean if contains a character
	{
		int count = 0;
		boolean first = false;
	...
	private String removeTrailingPunc(String word) //helper: returns the word with trailing punctuation removed
	{
		int count = 0;
		while (count < word.length())
		{
	...
	public ArrayList<String> top5search(String kw1, String kw2) {
		...
		ArrayList<Occurrence> ocurr1 = new ArrayList<Occurrence>();
		ArrayList<Occurrence> ocurr2 = new ArrayList<Occurrence>();
		ArrayList<Occurrence> combined = new ArrayList<Occurrence>();
		
		if (keywordsIndex.containsKey(kw1)){
			ocurr1 = keywordsIndex.get(kw1);
		}
		if (keywordsIndex.containsKey(kw2)){
			ocurr2 = keywordsIndex.get(kw2);
		}
		combined.addAll(ocurr1);
		combined.addAll(ocurr2);
		
		if (!(ocurr1.isEmpty()) && !(ocurr2.isEmpty())){
